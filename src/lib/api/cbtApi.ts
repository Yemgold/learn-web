...
export interface CreatedCbtSession {
  ...
  /*
   * True when the backend rejected the session because
   * the user's wallet balance is insufficient.
   *
   * This is a normal business condition, not a
...
export async function createCbtSession(
  ...
  try {
    ...
    if (
      ...
      console.warn(
        "[CBT API] Insufficient wallet balance.",
      );

      /*
       * This is a valid business response from
       * the backend, not a technical failure.
       *
       * Return a normalized result so the
       * practice page can handle it gracefully.
       */
