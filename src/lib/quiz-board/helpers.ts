...
export function getWinnerReward(
  ...
  return (
    getNumberValue(quiz.winner_reward) ||
    getNumberValue(quiz.winnerReward) ||
    getNumberValue(quiz.first_reward) ||
    getNumberValue(quiz.firstReward)
  );
}
...
export function getSecondReward(
  ...
  return (
    getNumberValue(quiz.second_reward) ||
    getNumberValue(quiz.secondReward)
  );
}
...
export function mapQuizToBoard(
  ...
  const contestantId =
    getStringValue(quiz.contestantId) ||
    getStringValue(quiz.contestant_id) ||
    null;
  ...
  const roomStatus =
    getStringValue(quiz.room_status) ||
    getStringValue(quiz.roomStatus) ||
    null;

  const currentRound =
    getNumberValue(quiz.current_round) ||
    0;
  ...
  return {
    ...
    /**
     * Required by QuizBoard.
     *
     * Admin competitions normally have no contestant ID,
     * so this correctly becomes null.
     */
    contestantId,

    /**
     * Preserve backend room information when available.
     */
    roomStatus,

    /**
     * Preserve the backend's current round.
     */
    currentRound,
  };
}
